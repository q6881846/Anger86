...
# Fix: Chinese quotes inside JSON string values must be escaped
# Left double quote U+201C → escaped ASCII quote \"
# Right double quote U+201D → escaped ASCII quote \"
raw_fixed = raw.replace("\u201c", '\\"').replace("\u201d", '\\"')

try:
    data = json.loads(raw_fixed)
    print("JSON parse success after fixing Chinese quotes!")

    # Update steps 6-8 with new prompts (no anti-add rules, new worldview format)
...
```"""
    ...
    # Write back as proper JSON (json.dump handles escaping)
    with open(path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

    print("SUCCESS: All steps updated!")
    print(f"Step 6: {len(data['step6']['user'])} chars")
    print(f"Step 7: {len(data['step7']['user'])} chars")
    print(f"Step 8: {len(data['step8']['user'])} chars")
